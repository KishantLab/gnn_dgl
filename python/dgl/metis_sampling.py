...
def metis_partition(G, parts=None, method=None, spmm_reorderd=0, sampling=0, dataset=None):
    ...
    if _computed_array is None and sampling == 0:
        ...
        if method is None:
                ...
                ##------------------------------start the sorting------------------------------------------------
                if os.path.exists(dataset + "_metis_part_" + part + "_sorted.pth"):
                 # _computed_array = np.load(dataset + "_metis_part_" + part + "_sorted.pth")
                  _computed_array = torch.load(dataset + "_metis_part_" + part + "_sorted.pth")
                  _computed_array = _computed_array.to(device)
                ...
                ##----------------------------------end the sorting------------------------------------------------
                   ...
                   # # b = np.array([4, 5, 6])
                    ...
                    ##---------------------------strat the sorting------------------------------------------------
                     ...
                     # import torch
                     torch.save(_computed_array, dataset + "_metis_part_" + part + "_sorted.pth")
