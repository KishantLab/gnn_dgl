...
echo -e "\tSPMM\tSampling\tFor_loop_time\tModel_time\tLoss Time\tTotal_time\tAccuracy"
...
for B in "${batch_sizes[@]}"; do
    ...
    for dataset in "${datasets[@]}"; do
	    ...
	    accuracy=$(tail -n 3 "$file" | cut -d ' ' -f3)
            echo -e "$spmm\t$sampling\t$for_loop\t$model\t$loss\t$total\t$accuracy"
        else
            echo -e "N/A\tN/A\tN/A\tN/A\tN/A\tN/A\tN/A"
        fi
    done
done
