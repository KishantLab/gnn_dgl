...
for fanout in "${fanouts[@]}"; do
  ...
  for batch_size in "${batch_sizes[@]}"; do
    ...
    echo -e "\n▶️ Running config: Start Time $(date +"%Y-%m-%d %H:%M:%S") | Dataset=$dataset | Fanout=$fanout | Batch=$batch_size | Epoch=$epoch | gespmm"
    ...
    if [ "$dataset" == "yelp" ]; then
      ...
      output=$(python3 node_classification_yelp.py --dataset=$1 --batch_size=$batch_size --fan_out=$fan_out_arg --epoch=$2 --spmm=gespmm --sampling default --mode puregpu)
    else
	output=$(python3 node_classification.py --dataset=$1 --batch_size=$batch_size --fan_out=$fan_out_arg --epoch=$2 --spmm=gespmm --sampling default --mode puregpu)
    fi
 
    # output=$(python3 node_classification.py --dataset=$1 --batch_size=$batch_size --fan_out=$fanout,$fanout,$fanout --epoch=$2 --spmm=gespmm --sampling default --mode puregpu)
    ...
    if [ "$num_layer" -gt 3 ]; then
      # echo "num_layer is greater than 3, setting fan_out_arg to $fan_out_arg"
      filename="training_time/$1/$1_F${fanout}_B${batch_size}_${epoch}_Sampling_gespmm_puregpu_layer-${num_layer}.txt"
    else
	filename="training_time/$1/$1_F${fanout}_B${batch_size}_${epoch}_Sampling_gespmm_puregpu.txt"
    fi
    echo "Dataset = $1, batch_size = $batch_size" > $filename
    last_spmm_time=$(echo "$output" | tac | grep -m1 "^ge-spmm time" | awk '{print $3}')
