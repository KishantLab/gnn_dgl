...
for fanout in "${fanouts[@]}"; do
  ...
  for batch_size in "${batch_sizes[@]}"; do
    ...
    output=$(python3 node_classification.py --dataset=$1 --batch_size=$batch_size --fan_out=$fanout,$fanout,$fanout --epoch=$2 --method=contig)
    filename="training_time/contig/$1/$1_F${fanout}_B${batch_size}_${epoch}_SMEM.txt"
