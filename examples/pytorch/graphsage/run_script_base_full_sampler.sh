...
# output=$(python3 node_classification.py --d1ataset=$1 --batch_size=1024)
...
for fanout in "${fanouts[@]}"; do
  ...
  for batch_size in "${batch_sizes[@]}"; do
    ...
    output=$(python3 node_classification.py --dataset=$1 --batch_size=$batch_size --fan_out=$fanout,$fanout,$fanout --epoch=$2 --sampler full)

        # Kill the progress spinner
    kill $progress_pid >/dev/null 2>&1
    wait $progress_pid 2>/dev/null

    filename="training_time/$1/$1_F${fanout}_B${batch_size}_${epoch}_Sampling_default_full_sampler.txt"
