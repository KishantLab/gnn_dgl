...
//--------------skip the vertices which is sampled earlier------------------------//
  ...
  while (out_row < last_row) {
	...
	__shared__ int actual_select_ptr;      // real count of selected
	__shared__ int actual_select_non_ptr;  // real count of non-part selected
	...
	if(threadIdx.x == 0)
	{
	// int deg = in_row_end - in_row_start;
	// float alpha_v = fminf(1.0f, (float)128 / (float)deg); // c = 64 or fanout
	// copy_ptr = (int)(alpha_v * num_picks); // number of picks from same partition
	// int k1 = (int)(alpha_v * num_picks); // number of picks from same partition
	// int k2 = num_picks - k1;
	select_ptr = 0; // pointer to store the selected vertices
	select_non_part_ptr = 0; // pointer to store the selected non-partition vertices
	...
	// for(int i = threadIdx.x + in_row_start; i < in_row_end; i += METIS_BLOCK_SIZE)
	{
	  ...
	  // if(Neb_part == seed_node_part)
	  {
	    ...
	    // {
	    //   neb_id_list[loc] = Neb;
	    //   neb_id_list[loc + num_picks] = i;
	    //   atomicAdd(&actual_select_ptr, 1);  // increment only if stored
	    // }
	    neb_id_list[select_ptr] = Neb;
	    neb_id_list[select_ptr + num_picks] = i;
	    select_ptr++;
	  }  //end if
	  ...
	  // else
	  {
	     // int loc_non_part = atomicAdd(&select_non_part_ptr, 1);
	     // if(loc_non_part < num_picks)
	     // {
		 // non_part_neb[loc_non_part] = Neb;
		 // non_part_neb[loc_non_part + num_picks] = i;
		 // atomicAdd(&actual_select_non_ptr, 1);  // increment only if stored
	     // }
	...
	// if (copy_ptr == select_ptr && num_picks == select_non_part_ptr)
	// if (num_picks <= actual_select_ptr && num_picks <= actual_select_non_ptr)
	// if (num_picks <= actual_select_ptr)
	{	
		...
		//   out_idxs[out_row_start + idx + copy_ptr] = data ? data[non_part_neb[num_picks + idx]] : non_part_neb[num_picks + idx];
		...
		{ 
		    ...
		    else if (idx >= copy_ptr && idx < num_picks)
		    {
			// for(int idx = threadIdx.x; idx < (num_picks - copy_ptr); idx += METIS_BLOCK_SIZE)
			// { 
		  	// copy the indices which is alredy sorted based on partition and degree
		  	...
		  	out_idxs[out_row_start + idx] = data ? data[non_part_neb[num_picks + idx - copy_ptr]] : non_part_neb[num_picks + idx - copy_ptr];
		  	// out_cols[out_row_start + idx + copy_ptr] = d_part_array[idx + in_row_start];
		  	// out_idxs[out_row_start + idx + copy_ptr] = data ? data[idx] : idx;
		    }
		  }
	...
	{
		// for(int idx = threadIdx.x; idx < actual_select_ptr; idx += METIS_BLOCK_SIZE)
		...
		// // int remining_size = num_picks - actual_select_ptr;
		...
		//   out_idxs[out_row_start + idx + select_ptr] = data ? data[non_part_neb[num_picks + idx]] : non_part_neb[num_picks + idx];
		//   // out_cols[out_row_start + idx] = d_part_array[idx + in_row_start];
		//   // out_idxs[out_row_start + idx] = data ? data[idx] : idx;
		// }
		// for(int idx = threadIdx.x; idx < actual_select_ptr; idx += METIS_BLOCK_SIZE)
		...
		{ 
		  ...
		  else if (idx >= select_ptr && idx < num_picks)
		  {
		  	// copy the indices which is alredy sorted based on partition and degree
			  const IdType in_idx = in_row_start + idx;
			  out_rows[out_row_start + idx] = row;
			  // out_cols[out_row_start + idx + select_ptr] = neb_id_list[idx + num_picks];
			  out_cols[out_row_start + idx] = non_part_neb[idx - select_ptr];
			  // out_cols[out_row_start + idx + select_ptr] = d_part_array[in_idx];
			  out_idxs[out_row_start + idx] = data ? data[non_part_neb[num_picks + idx - select_ptr]] : non_part_neb[num_picks + idx - select_ptr];
			  // out_cols[out_row_start + idx] = d_part_array[idx + in_row_start];
			  // out_idxs[out_row_start + idx] = data ? data[idx] : idx;
		  }
		}
